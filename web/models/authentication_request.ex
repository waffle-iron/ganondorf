defmodule Ganondorf.AuthenticationRequest do
  ...
  @aloud_keys Keyword.keys(@structure) -- [:flow_type]
  ...
  ## Examples
      ...
      iex> Ganondorf.AuthenticationRequest.new(%{flow_type: :great})
      %Ganondorf.AuthenticationRequest{flow_type: :invalid}
  """
  @spec new(Map) :: __MODULE__.t
  def new(params \\ %{}) when is_map(params) do
    params
    |> extract_aloud_params
    |> populate_request
    |> set_flow_type
  end
  ...
  defp extract_aloud_params(params) do
    Enum.map(@aloud_keys, fn key ->
     {key, Map.get(params, key) || Map.get(params, "#{key}", @structure[key])}
    end)
  end

  defp populate_request(params) do
    Enum.reduce(params, %__MODULE__{}, fn { key, value }, acc ->
      Map.put(acc, key, "#{value}")
    end)
  end

  defp set_flow_type(%__MODULE__{} = request) do
    %{request | flow_type: flow_type(request)}
  end
end
